...
class Hopfield:
    def __init__(self, n, shape=None):
        ...
        self.shape = shape  # para mostrar las iteraciones
    ...
    def train_pseudoinverse(self, patterns):
        X = np.stack(patterns, axis=0).astype(np.float64)  
        G = X @ X.T                                        
...
if __name__ == "__main__":
    folder = "dataset_notas"   
    clf = HopfieldClassifier(rule="pseudoinverse")  
    ...
    #meter archivo de prueba
    test_file = "../test_pattern2.txt"  
    #test_file = "dataset_notas/bemol.txt"  
    try:
